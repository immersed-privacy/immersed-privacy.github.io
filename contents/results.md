...
Results expose a systematic **alignment bias toward task completion**. TCR consistently exceeds 0.75 for all competent models, while PPR hovers near the 0.67 random level for most. Only `gemini-3.1-pro` meaningfully breaks away (PPR = 0.93, EM = 0.51). When a benign command conflicts with an implicitly established privacy boundary, models default to carrying out the command.
...
An ablation replacing audio with semantic text descriptions confirms that **audio perception is not the bottleneck** — the alignment failures persist even when the perceptual demand of audio comprehension is removed entirely.
